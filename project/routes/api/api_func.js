...
function calcMonth(start, end) {
  const startTime = moment(start).format('YYYY-MM');
  const endTime = moment(end).format('YYYY-MM');
...
function categorizeNewsAsMonth(start, end, data) {
  if (data == false) {
    ...
    const timeCat = calcMonth(start, end);
    for (let i = 0; i < data.length; i++) {
      // 將日期的規格 YYYY/MM/DD 轉換為 YYYY-MM-DD
      const t = moment((data[i].date).replace(/\//g, '-')).format('YYYY-MM');
...
function categorizeNewsAsDate(start, end, data) {
  if (data == false) {
    ...
    const timeCat = calcDate(start, end);
    for (let i = 0; i < data.length; i++) {
      // 將日期的規格 YYYY/MM/DD 轉換為 YYYY-MM-DD
      const t = moment((data[i].date).replace(/\//g, '-')).format('YYYY-MM-DD');
      timeCat[t].news.push(data[i]);
      timeCat[t].totalScore += Number(data[i].score);
      timeCat[t].totalMag += Number(data[i].magnitude);
    }
...
function comparison(cna, media) {
  if (media == false) {
    ...
    for (let i = 0; i < media.length; i++) {
      const compare = calcSimilarity((cna.tokenize).split(','), (media[i].tokenize).split(','));
      if (compare > max) {
        max = compare;
        result = media[i];
      }
...
function calcSimilarity(str1, str2) {
  ...
  const s = similarity(str1_vec, str2_vec);
  return s;
}
module.exports = {
  calcMonth: calcMonth,
