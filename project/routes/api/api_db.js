...
function getNews_news(media, input, start, end) {
  return new Promise((resolve, reject) => {
    ...
    SELECT n.*, a.tokenize 
    FROM tbt.news AS n 
    INNER JOIN tbt.article AS a 
    ON n.media = ? 
    AND a.news_url=n.url
    `;
    for (let i = 0; i < input.length; i++) {
      sql += `AND a.article LIKE ?`;
    }
    sql += `
    AND TO_DAYS(n.date)>= TO_DAYS(?) 
    AND TO_DAYS(n.date) < TO_DAYS(?)
    `;
    ...
    mysql.query(sql, set, function (err, result) {
      ...
      if (result.length == 0) {
        ...
        for (let i = 0; i < result.length; i++) {
          const reporterSql = `SELECT r.name FROM reporter AS r, reporter_has_news AS rn WHERE rn.news_id=? AND r.id = rn.reporter_id`;
...
function getNews_reporter(media, input, start, end, name) {
  return new Promise((resolve, reject) => {
    ...
    SELECT n.* 
    FROM tbt.news AS n 
    INNER JOIN tbt.reporter_has_news AS rn  
    ON n.id = rn.news_id 
    INNER JOIN tbt.reporter AS r 
    ON rn.reporter_id = r.id 
    AND r.name=? 
    INNER JOIN tbt.article AS a 
    ON n.url = a.news_url 
    `;
...
function getReporterName() {
  return new Promise((resolve, reject) => {
    const sql = `SELECT * FROM reporter`;
